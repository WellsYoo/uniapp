...
@implementation HMYAppManager {
    NSMutableDictionary *_sessions; /**< 存储应用程序请求 */
    NSMutableDictionary *_actions;  /**< 存储应用程序事件 */
    NSMutableDictionary *_xActions;
}

+ (HMYAppManager *)sharedManager {
    __strong static HMYAppManager *sharedObj = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedObj = [[HMYAppManager alloc] init];
    });
...
- (BOOL)handleOpenURL:(NSURL *)url {
    ...
    if ([action isEqualToString:kMTResponse]) {
        ...
        HMYAppRequest *request = _sessions[requestID];
        if (request) {
            // 获取该平台目前所支持的所有api
 ...
 *	@param 	request 	请求对象
 */
- (void)sendRequest:(HMYAppRequest *)request {
    // 应用程序未安装
...
+ (void)openAppWithId:(NSString *)appId viewController:(UIViewController *)viewController {
    [HMYAppManager openAppWithId:appId viewController:viewController delegate:nil];
}
