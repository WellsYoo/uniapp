...
#import <MPITextKit/MPITextViewModel.h>
...
@interface MPIAttributedLabel : UIView <MPIAttributedTextView>

/** Delegate to receive tap and long press events. */
@property (nullable, nonatomic, weak) id<MPIAttributedTextViewDelegate> delegate;

/** `NSAttributedString` attributes applied to links when touched.*/
@property (nullable, nonatomic, copy) NSDictionary<NSString *, id> *activeLinkTextAttributes;
...
/**
 The underlying attributed string drawn by the label, if set, the label ignores other properties.
 */
...
/**
 A UIBezierPath representing the exclusion path inside the receiver's bounding rect.
 The default value is empty.
 */
@property (nullable, nonatomic, copy) UIBezierPath *exclusionPath;
