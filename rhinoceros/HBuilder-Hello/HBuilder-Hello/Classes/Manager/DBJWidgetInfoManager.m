...
@interface DBJWidgetInfoManager()
...
@implementation DBJWidgetInfoManager
    ...
    if (!_personList) {
        ...
        for (NSDictionary * item in infoList) {
            ...
            switch (type) {
                ...
                {
                    DBJWidgetPerson * person = [[DBJWidgetPerson alloc] initWithData:item];
                    if (person) {
...
- (BOOL)saveToWidget
{
    ...
    for (id item in self.personList) {
        if ([item isKindOfClass:[DBJWidgetPerson class]]) {
            DBJWidgetPerson *person = (DBJWidgetPerson *)item;
            NSDictionary * personInfo = [person personInfoForWidget];
            [infoList addObject:personInfo];
        }else {
            DBJApplicationItem *appItem = (DBJApplicationItem *)item;
            NSDictionary *appInfo = [appItem applicationInfoForWidget];
            [infoList addObject:appInfo];
        }
    }
    NSURL * contentURL = [[NSFileManager defaultManager] containerURLForSecurityApplicationGroupIdentifier:[DBJWidgetInfoManager groupIdentifier]];
...
- (BOOL)addNCWItemToWidget:(id)ncwItem saveToWidget:(BOOL)save
{
    ...
    if (contain && [ncwItem isKindOfClass:[DBJWidgetPerson class]] ) {
...
- (BOOL)deleteFromWidget:(id )ncwItem saveWidget:(BOOL)save
{
    ...
    [self.personList enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:[DBJWidgetPerson class]]&&[ncwItem isKindOfClass:[DBJWidgetPerson class]]) {
            
            DBJWidgetPerson * tmpPerson = (DBJWidgetPerson *)obj;
            if ( [tmpPerson isEqualToPerson:(DBJWidgetPerson *)ncwItem]) {
                index = idx;
                *stop = YES;
            }

        }else if([obj isKindOfClass:[DBJApplicationItem class]]&&[ncwItem isKindOfClass:[DBJApplicationItem class]]){
...
- (BOOL)__widgetContainsNcwItem:(id)ncwItem
{
    ...
    [self.personList enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        {
            DBJApplicationItem * tmpApp = (DBJApplicationItem *)obj;
...
- (void)requestApplicationInfo:(DBJApplicationItem *)appItem complete:(NCWApplicationInfoResponse)applicationInfo
{
...
{
    DBJApplicationItem *appItem = [requestInfo objectForKey:ApplicationRequestQueueAppItem];
