...
- (void)performWithURLString:(NSString *)urlString
    ...
    if (url && [self isSupportPerformURLString:urlString]) {
        ...
        MTAppRequest *request = [[MTAppRequest alloc] init];
        ...
        MTAppRequestMessage *message = [[MTAppRequestMessage alloc] initWithImage:requestMessage.image requestData:requestData];
        request.client          = self;
        ...
        if (self.manager) {
            ...
            [[MTAppManager sharedManager] sendRequest:request];
        }
    } else {
        if (failure) {
