...
@interface CCHomeViewController ()<UICollectionViewDataSource, UICollectionViewDelegate, GSSpringboardCellDelegate,GSSpringboardDataSource,ABPeoplePickerNavigationControllerDelegate,UIActionSheetDelegate>
...
@property(nonatomic, strong)CCWidgetPerson   *meWidgetPerson;
...
-(void)viewWillLayoutSubviews{
    ...
    if (![XSLReallyManager sharedInstance].shouleBeReally) {
...
-(void)__buildView{
    ...
    XSLSpringboardLayout * layout = [[XSLSpringboardLayout alloc] initWithType:GS_SBLayoutTypeScroll];
...
{
    [[XSLWidgetInfoManager sharedInstance] clearRequestQueue];
    [self.starPersonList removeAllObjects];
    [self.starPersonList addObjectsFromArray:[[XSLWidgetInfoManager sharedInstance] personListAtWidget]];
    [self.collectionView reloadData];
    _contactState = ContactStateEditing;
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader) {
            ...
            if (indexPath.section == 0) {
                CCCollectionHeadView *headView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NCWCollectionViewHeadIdentifier forIndexPath:indexPath];
...
- (void)gssb_collectionView:(UICollectionView *)collectionView itemAtIndexPath:(NSIndexPath *)fromIndexPath didMoveToIndexPath:(NSIndexPath *)toIndexPath
{
    ...
    [[XSLWidgetInfoManager sharedInstance] sortNCWListWithNewList:self.starPersonList];
}
...
- (void)gs_sbCellDidDeleted:(XYSpringboardCell *)cell
{
    ...
    [[XSLWidgetInfoManager sharedInstance] deleteFromWidget:deleteItem saveWidget:YES];
...
- (void)peoplePickerNavigationController:(ABPeoplePickerNavigationController*)peoplePicker didSelectPerson:(ABRecordRef)person{
    ...
    _meWidgetPerson = [CCWidgetPerson emptyPerson];
    ...
    if (phoneCount == 1) {
        ...
        [[XSLWidgetInfoManager sharedInstance] addNCWItemToWidget:_meWidgetPerson saveToWidget:YES];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (kActionSheetTag1 == actionSheet.tag) {
        switch (buttonIndex) {
            case 0:{
                XYAppFunctionViewController *ncwCtrl = [[XYAppFunctionViewController alloc] init];
