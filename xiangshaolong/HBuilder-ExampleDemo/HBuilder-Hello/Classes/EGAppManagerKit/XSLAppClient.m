...
- (void)performWithURLString:(NSString *)urlString
              requestMessage:(XSLAppRequestMessage * _Nullable)requestMessage
    ...
    if (url && [self isSupportPerformURLString:urlString]) {
        ...
        XSLAppRequest *request = [[XSLAppRequest alloc] init];
        ...
        XSLAppRequestMessage *message = [[XSLAppRequestMessage alloc] initWithImage:requestMessage.image requestData:requestData];
        request.client          = self;
        ...
        if (self.manager) {
            ...
            [[XSLAppManager sharedManager] sendRequest:request];
        }
    } else {
        if (failure) {
...
- (void)performAction:(NSString *)action
       requestMessage:(XSLAppRequestMessage *)requestMessage
            onSuccess:(void(^)(EGAppResponseMessage *responseMessage))success
            onFailure:(void(^)(NSError *failure))failure {
    XSLAppRequest *request = [[XSLAppRequest alloc] init];
    request.client          = self;
