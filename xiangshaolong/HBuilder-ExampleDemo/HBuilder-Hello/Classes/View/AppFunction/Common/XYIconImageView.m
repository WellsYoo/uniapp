...
- (void)setNCWAppItem:(YXpplicationItem *)appItem
{
    ...
    if (_appItem.isFromInstalled) {
        if (_appItem.isLoaded) {
            ...
            [[XSLWidgetInfoManager sharedInstance] requestApplicationInfo:_appItem complete:^(NSDictionary *appInfo){
                if (appInfo) {
...
-(void)setNCWPersonImage:(CCWidgetPerson *) person{
    ...
    if (_person.imageData) {
    ...
    }else if([[XSLWidgetInfoManager sharedInstance] onlyContainChineseCharacter:fullName]&&fullName.length > 0){
        ...
        XSLNWCView *drawTextView = [[XSLNWCView alloc] initWithFrame:CGRectMake(0, 0, 42, 42) Text:lastCharacter Letter:false];
        UIImage *image = [[XSLWidgetInfoManager sharedInstance] convertViewToImage:drawTextView];
        if (image) {
            self.image = image;
        }
    }else if ([[XSLWidgetInfoManager sharedInstance] onlyContainLetter:fullName]&&fullName.length > 0){
        NSString *showName = [fullName stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        NSString *firstLetter = [[showName substringToIndex:1] uppercaseString];
        XSLNWCView *drawTextView = [[XSLNWCView alloc] initWithFrame:CGRectMake(0, 0, 42, 42) Text:firstLetter Letter:YES];
        UIImage *image = [[XSLWidgetInfoManager sharedInstance] convertViewToImage:drawTextView];
        if (image) {
            self.image = image;
        }
        ...
        self.image = [[XSLWidgetInfoManager sharedInstance] defaultPortrait];
    }
}
