//
//  UIScrollView+MTKeyboardAvoidingAdditions.m
//  MTKeyboardAvoiding
//
...
@interface MTKeyboardAvoidingState : NSObject
...
@implementation UIScrollView (MTKeyboardAvoidingAdditions)

- (MTKeyboardAvoidingState*)keyboardAvoidingState {
    MTKeyboardAvoidingState *state = objc_getAssociatedObject(self, &kStateKey);
    if ( !state ) {
        state = [[MTKeyboardAvoidingState alloc] init];
...
- (void)MTKeyboardAvoiding_keyboardWillShow:(NSNotification*)notification {
    ...
    MTKeyboardAvoidingState *state = self.keyboardAvoidingState;
    ...
    if ( [self isKindOfClass:[MTKeyboardAvoidingScrollView class]] ) {
    ...
    dispatch_after(delay, dispatch_get_main_queue(), ^{
        ...
        UIView *firstResponder = [self MTKeyboardAvoiding_findFirstResponderBeneathView:self];
        if ( firstResponder ) {
            
            self.contentInset = [self MTKeyboardAvoiding_contentInsetForKeyboard];
            
            CGFloat viewableHeight = self.bounds.size.height - self.contentInset.top - self.contentInset.bottom;
            [self setContentOffset:CGPointMake(self.contentOffset.x,
                                               [self MTKeyboardAvoiding_idealOffsetForView:firstResponder
...
- (BOOL)MTKeyboardAvoiding_focusNextTextField {
    ...
    UIView *view = [self MTKeyboardAvoiding_findNextInputViewAfterView:firstResponder beneathView:self];
...
- (UIView*)MTKeyboardAvoiding_findFirstResponderBeneathView:(UIView*)view {
    ...
    for ( UIView *childView in view.subviews ) {
        ...
        UIView *result = [self MTKeyboardAvoiding_findFirstResponderBeneathView:childView];
        if ( result ) return result;
    }
    return nil;
}

- (UIView*)MTKeyboardAvoiding_findNextInputViewAfterView:(UIView*)priorView beneathView:(UIView*)view {
    UIView * candidate = nil;
    [self MTKeyboardAvoiding_findNextInputViewAfterView:priorView beneathView:view bestCandidate:&candidate];
    return candidate;
}

- (void)MTKeyboardAvoiding_findNextInputViewAfterView:(UIView*)priorView beneathView:(UIView*)view bestCandidate:(UIView**)bestCandidate {
    ...
    CGFloat bestCandidateHeuristic = [self MTKeyboardAvoiding_nextInputViewHeuristicForViewFrame:candidateFrame];
    
    for ( UIView *childView in view.subviews ) {
        if ( [self MTKeyboardAvoiding_viewIsValidKeyViewCandidate:childView] ) {
            CGRect frame = [self convertRect:childView.frame fromView:view];
            
            // Use a heuristic to evaluate candidates
            CGFloat heuristic = [self MTKeyboardAvoiding_nextInputViewHeuristicForViewFrame:frame];
            ...
            [self MTKeyboardAvoiding_findNextInputViewAfterView:priorView beneathView:childView bestCandidate:bestCandidate];
        }
    }
}

- (CGFloat)MTKeyboardAvoiding_nextInputViewHeuristicForViewFrame:(CGRect)frame {
    return  (-frame.origin.y * 1000.0) // Prefer elements closest to top (most important)
          + (-frame.origin.x);         // Prefer elements closest to left
}

- (BOOL)MTKeyboardAvoiding_viewHiddenOrUserInteractionNotEnabled:(UIView *)view {
    while ( view ) {
...
- (BOOL)MTKeyboardAvoiding_viewIsValidKeyViewCandidate:(UIView *)view {
    if ( [self MTKeyboardAvoiding_viewHiddenOrUserInteractionNotEnabled:view] ) return NO;
...
- (void)MTKeyboardAvoiding_assignTextDelegateForViewsBeneathView:(UIView*)view {
    for ( UIView *childView in view.subviews ) {
        if ( ([childView isKindOfClass:[UITextField class]] || [childView isKindOfClass:[UITextView class]]) ) {
            [self MTKeyboardAvoiding_initializeView:childView];
        } else {
            [self MTKeyboardAvoiding_assignTextDelegateForViewsBeneathView:childView];
        }
    }
}
...
- (UIEdgeInsets)MTKeyboardAvoiding_contentInsetForKeyboard {
    MTKeyboardAvoidingState *state = self.keyboardAvoidingState;
...
-(CGFloat)MTKeyboardAvoiding_idealOffsetForView:(UIView *)view withViewingAreaHeight:(CGFloat)viewAreaHeight {
...
- (void)MTKeyboardAvoiding_initializeView:(UIView*)view {
    if ( [view isKindOfClass:[UITextField class]]
        ...
        UIView *otherView = [self MTKeyboardAvoiding_findNextInputViewAfterView:view beneathView:self];
...
@implementation MTKeyboardAvoidingState
@end
